        ...
        return new Promise((resolve, reject) => {
            ...
            Promise.all(loadPromises).then(() => {
                ...
                if (imageTextures.length > 0) {
                    ...
                    reject(new Error('テクスチャのロードに失敗しました。'));
                }
            });
        });
    }
...
function updateBlurEffect(scrollPosition) {
    ...
    if (scrollPosition > blurStartThreshold && scrollPosition <= blurEndThreshold) {
        ...
        brightnessAmount = 1 + progress * (MAX_BRIGHTNESS - 1); // 明度の計算を追加
    ...
    blurOverlay.style.webkitBackdropFilter = filterValue; // Safari support

    // 彩度と明度の効果を背景キャンバスに適用
    if (backgroundCanvas) {
        backgroundCanvas.style.filter = `saturate(${saturationAmount.toFixed(2)}) brightness(${brightnessAmount.toFixed(2)})`;
    }
}
...
function updateProjectBackground(newIndex) {
    ...
    if (newIndex === -1) {
        gsap.to(backgroundCanvas, {
            ...
            onComplete: () => {
                if (backgroundCanvas) {
                    backgroundCanvas.style.filter = ''; // 彩度をリセット
                }
            }
        });
        if (currentBg) {
